...
async function cargarTrendencias() {
    try {
        ...
        // El backend devuelve un objeto { totalItemsAnalyzed, topKeywords: [{keyword, occurrences}], sentimentBreakdown: {...} }
        const trending = await response.json();
        const keywords = trending?.topKeywords || [];
        ...
        keywords.slice(0, 6).forEach(trend => {
            const card = document.createElement("div");
            card.className = "trending-card";

            card.innerHTML = `
                <div class="trending-keyword">${trend.keyword}</div>
                <div class="trending-count">${trend.occurrences}</div>
        ...
        // Resumen de sentimiento agregado (positivo/negativo/neutral) sobre todos los items analizados
        const sentimientos = trending?.sentimentBreakdown || {};
        const etiquetas = { positivo: "😊 Positivo", negativo: "😞 Negativo", neutral: "😐 Neutral" };
        const resumen = Object.entries(sentimientos)
            .map(([clave, valor]) => `<span style="margin-right: 14px;">${etiquetas[clave] || clave}: <strong>${valor}</strong></span>`)
            .join("");

        if (resumen) {
            trendingContainer.insertAdjacentHTML("beforeend", `
                <div style="grid-column: 1/-1; text-align: center; margin-top: 14px; font-size: 13px; color: #6b7280;">
                    ${resumen}
                </div>
            `);
        }
